...
- **Shared Schemas**: Author canonical schemas in a shared `/contracts` package (e.g., `zod`) and import them in both API handlers and React forms to stay in sync
- **Third-Party Payloads**: Treat every upstream webhook/SDK payload as untrusted—parse and validate against schemas before touching business logic
